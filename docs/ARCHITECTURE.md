...
> `backend/scripts/generate_architecture_png.py` (run from the repo root as
> `python backend/scripts/generate_architecture_png.py`).
...
`backend/app/api/routes.py` exposes three REST endpoints:
...
### Agents Config — `backend/agents/`

One Python module per platform agent (e.g. `backend/agents/defect_triage_crewai.py`). Each declares:
...
| Agents config in `backend/agents/<slug>.py` | Adding a new agent requires one file + one registry line, nothing else |
